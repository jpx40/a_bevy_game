...
impl ClientDataUploader {
    pub fn upload<T: Serialize>(&mut self, data: T, id: u32) -> Result<(), ConnectionError> {
...
impl ServerDataUploader {
    pub fn upload<T: Serialize>(&mut self, data: T, recipient: Recipient, id: u32) {
        let bytes = bincode::serialize(&data);
        match bytes {
            Ok(bytes) => {
                self.list.push((
...
pub fn client_specific_setup(mut commands: Commands) {
    println!("Client");
...
pub fn server_specific_setup(mut commands: Commands) {
    println!("Server");
                    ...
                    Err(err) => {
                        eprintln!(
                            "Encountered an error \
                                 while trying to connect to the server!: {err:#?}"
                        )
                    }
                    Ok(e) => match e {
                        Err(ConnectionError::ConnectionRefused) => {
                            println!(
                                "Unable\
                 to connect to the server because it refused the connection attempt."
                            )
                        }
                        Err(err) => {
                            eprintln!(
                                "Encountered an \
                                error while trying to connect to the server!: {err:#?}"
                            )
                        }
                        _ => {}
                    },
...
fn setup(client_config: Option<Res<ClientConfig>>, server_config: Option<Res<ServerConfig>>) {
    if let Some(_) = client_config {
        if let Some(_) = server_config {
            panic!(
                "Both client and server configs have been found!\
                The Application cannot be both a client and a server at the same time!"
            );
        }
    }
    if let None = client_config {
        if let None = server_config {
            panic!(
                "Both the Client Config and Server Config have not been set! 
                Please insert one as a resource at the build stage of your app"
            );
        }
    }
}
fn send_data_to_clients(
    server_streams: Option<Res<ServerStreams>>,
    mut server_data_uploader: Option<ResMut<ServerDataUploader>>,
) {
    if let Some(mut server_data_uploader) = server_data_uploader {
        if let Some(server_streams) = server_streams {
            ...
            for (recipient, data_packet) in owned_list {
                match recipient {
                    ...
                    Recipient::AllExcept { id } => {
                        for (key, stream) in server_streams.streams.lock().unwrap().iter() {
                            if *key != id {
                                if let Err(err) = send_data(data_packet.clone(), stream) {
                                    println!(
                                        "Received an error trying to send data packet: {err:#?}"
                                    );
                                }
                            }
                        }
                    ...
                    Recipient::Single { id } => {
                        match server_streams.streams.lock().unwrap().get(&id) {
                            Some(stream) => {
...
fn send_data_to_server(
    mut client_data_uploader: Option<ResMut<ClientDataUploader>>,
    client_stream: Option<Res<ClientStream>>,
) {
    if let Some(client_stream) = client_stream {
        let stream = client_stream.stream.lock().unwrap();
        let stream = stream.as_ref();
        if let Some(mut client_data_uploader) = client_data_uploader {
