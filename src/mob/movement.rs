...
use crate::{modifier::{self, Speed}, ui::UiAction};
...
#[derive(Bundle)]
pub struct MobControllerBundle {
    character_controller: MobController,
...
fn keyboard_input(
    ...
    mut ui_event: EventWriter<UiAction>,
...
fn update_grounded(
    ...
    mut query: Query<(Entity, &ShapeHits, &Rotation, Option<&MaxSlopeAngle>), With<MobController>>,
) {
...
fn movement(
    time: Res<Time>,
    mut movement_reader: Query<(&super::Move, &Speed, &mut Transform), With<Mob>>,
    ...
    for (event,s, mut t) in movement_reader.iter_mut() {
        for ( is_grounded) in
            &mut controllers
        {
            match event {
                super::Move::Left => {
                      t.translation.x += (-1. * s.0 * delta_time) * 2.;
                }
                super::Move::Right => {
                    t.translation.x += (1. * s.0 * delta_time) * 2.;
                }
...
fn kinematic_controller_collisions(
    ...
    mut character_controllers: Query<
        ...
        (With<RigidBody>, With<MobController>),
    >,
    time: Res<Time>,
) {
